...
UPDATE app_settings
   SET v = CAST(CAST(v AS INTEGER) * 20 AS TEXT)
...
UPDATE app_settings
   SET v = CAST(
             CAST((SELECT v FROM app_settings WHERE k = 'quota_per_usd') AS INTEGER)
             * 1000000
             / CAST((SELECT v FROM app_settings WHERE k = 'epay_quota_per_yuan') AS INTEGER)
           AS TEXT)
