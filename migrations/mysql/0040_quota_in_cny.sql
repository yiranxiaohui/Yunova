...
UPDATE app_settings
   SET `v` = CAST(CAST(`v` AS SIGNED) * 20 AS CHAR)
...
-- MySQL cannot read the table being updated in a subquery, so stage the
-- derived rate in a temporary table first.
CREATE TEMPORARY TABLE _rate AS
SELECT CAST(
         (SELECT CAST(`v` AS SIGNED) FROM app_settings WHERE `k` = 'quota_per_usd')
         * 1000000
         / (SELECT CAST(`v` AS SIGNED) FROM app_settings WHERE `k` = 'epay_quota_per_yuan')
       AS CHAR) AS rate
  FROM app_settings
 WHERE `k` = 'quota_per_usd'
   AND (SELECT CAST(`v` AS SIGNED) FROM app_settings WHERE `k` = 'epay_quota_per_yuan') > 0;

UPDATE app_settings
   SET `v` = (SELECT rate FROM _rate)
 WHERE `k` = 'usd_to_cny_rate_micro'
   AND (SELECT COUNT(*) FROM _rate) > 0;

DROP TEMPORARY TABLE _rate;
