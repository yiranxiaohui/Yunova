...
// 汇率与全局倍率换算成额度。默认汇率是 1:1（1 元额度买 1 美元上游用量）——
// 这不是外汇牌价，而是上游中转站自己的卖法（按列表美元价 1 美元收 1 元），
// 照真实汇率折算会把每个模型都按成本的好几倍计费。
...
/**
 * 站点默认汇率：1 美元上游用量 = 1 元额度。
 *
 * 与后端 `DEFAULT_USD_TO_CNY_MICRO` 一致，仅用于设置还没加载完时的预览兜底，
 * 免得界面先按别的倍数闪一下价格。
 */
export const DEFAULT_USD_TO_CNY_RATE_MICRO = MICRO_QUOTA
