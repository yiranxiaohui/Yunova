...
- **`existing_only` is the default mode in the UI**: a relay advertises hundreds of models while the operator sells a curated handful, so a resync refreshes exactly the rows already in `model_pricing` (`skipped_missing` counts the rest) and never creates new ones. It implies `overwrite_existing` because re-pricing is the point, but it only writes price columns — `enabled`, `display_name`, `context_limit`, `protocol` and channel bindings are read back from the existing row, so a model the admin disabled stays disabled. Local models the catalog never listed come back in `not_listed` instead of being silently stale, and a model whose local `kind` disagrees with the upstream billing mode is reported rather than rewritten, because zeroing the columns its billing path reads would serve it free.
