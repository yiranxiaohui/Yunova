...
桌面端能自己绑定，是因为它有窗口：客户端读**自己那个 webview** 的会话 cookie 去
证明这台机器。命令行没有窗口，而另外两条路都不能走——让用户在终端里输账号密码，
等于把「就是账号本身」的凭据放进终端、shell 历史和用户自己的磁盘；让用户手动贴
令牌，则把「登录」变成了一件需要自己管理凭据的杂活，而贴出去的长期令牌往往会被
遗忘在某个地方。

所以命令行走**设备码**：CLI 显示一个短码，用户拿到**浏览器**里确认，CLI 轮询到
批准后才拿到一枚普通的 Agent 令牌。批准发生在一个已经登录的会话里，这正是短码
安全的原因——光知道码没用，还必须有人登录着并点下同意。

两个码故意是不对称的：

- **`user_code`** 短，因为要人转抄，所以天然低熵：它靠「必须有已登录的批准者」和
  「十分钟过期」来兜底。字符表剔除了 `0/O`、`1/I/L`、`5/S`、`8/B`，否则一次登录
  会变成三次失败尝试。
- **`device_code`** 高熵，因为它才是最终换取令牌的凭据。库里只存它的 SHA-256，
  明文令牌只在**某一次**轮询里返回一次（`consumed_at` 让「一次」是字面意思），
  所以从日志里捞到的码无法被重放成一枚可用凭据。

| 接口 | 说明 |
| --- | --- |
| `POST /api/cli/auth/start` | 公开，创建登录码，返回 `device_code` 与 `user_code` |
| `POST /api/cli/auth/poll` | 公开，轮询；批准后**仅此一次**返回明文令牌 |
| `POST /api/cli/auth/code` | 需登录，查询待批准的码（谁在请求、哪台机器） |
| `POST /api/cli/auth/approve` | 需登录，批准并绑定到当前账号 |
| `POST /api/cli/auth/deny` | 需登录，拒绝 |
| `GET /api/cli/models` | 需令牌，返回该账号可用的模型与网关地址 |

批准页在 `/cli/login`，CLI 打印的链接已带上 `?code=`，所以常见路径是一次点击。
页面**不显示令牌**：令牌是在 CLI 自己的轮询里签发的，因此截图或旁人看屏都拿不到
任何东西。

`pi-package/` 是随仓库发布的 pi 扩展包，注册 `yunova-openai` 与 `yunova-claude`
两个 provider——分两个是因为 pi 按 provider 的 `api` 决定请求形状，而网关两种协议
都讲，合成一个会强迫所有模型走同一种线格式。模型列表不写死在包里，而是刷新时
...
会话列表按**工作目录分组**：工作任务的标题由首条提示自动生成，同一个标题会在多个
项目里重复出现，路径才是区分它们的那条信息。分组以目录名作标题（完整路径放在 tooltip，
因为 16rem 宽的栏里长路径只会被截成无用的前半截），组的顺序跟着组内最新一条走，
最近动过的项目仍然在最上面。普通对话没有目录，单独成组而不是硬塞进一个假目录；
云端任务和「本地电脑未指定目录」各自成组，后者按设备区分。组可以折叠，
偏好存在 `localStorage`。只有一个组时不显示组标题——那正是分组前的样子。
搜索时忽略折叠状态，否则被折起来的命中会显示成「没有结果」。
...
- **卡片必须显示具体要执行什么**。不展示命令的审批框不是安全措施，只会训练用户
  闭眼点「允许」。命令用等宽字体渲染并保留换行，因为引号、斜杠、重定向恰恰是
  决定它危不危险的那几个字符；万一某个客户端没附上内容，卡片会直说「无法显示」
  并建议先拒绝，而不是画一个看上去像坏了的空框。
...
它还有一块**原生菜单**（`新建对话` / `新建工作任务` / `本机设置` / `重新载入`，
带 `Cmd/Ctrl+N`、`Cmd/Ctrl+,` 等快捷键）。菜单项故意很薄，只是开窗口和导航站点
路径：产品功能活在页面里，在这里重实现一份就等于要维护两个版本。但「编辑」
菜单是**必须**的：macOS 只有在菜单声明了剪贴快捷键时才会把它们交给 webview，
缺了这块用户在页面里根本粘贴不了。
...
能在本机执行任务的客户端，就得能弄到「负责执行的那个东西」。之前它直接假定 `pi`
在 `PATH` 里，而这对双击图标装应用的人是错的。`desktop/src/runtime_install.rs` 分开
处理两件事：

- **找得到**。GUI 应用由窗口服务器拉起，不继承 shell 的 `PATH`；macOS 上从 Finder
  启动只能看到 `/usr/bin:/bin` 一类最小集。所以用户刚在终端里装好的运行时，在应用
  看来却「不存在」——这是这个问题最让人困惑的一半，因此除了 `PATH` 还会搜 `~/.bun/bin`、
  Homebrew、npm 全局前缀与各类版本管理器的 shims。
- **装得上**。真的没装时，面板上直接用 bun / pnpm / npm（按此优先级，取真实存在的
  那个）装到用户自己的前缀里。走包管理器而不是自己下二进制，是因为 pi 发布在 npm 上，
  自己实现一套安装会得到一个用户日后无法用惯用工具升级的副本。

安装方式只能从固定白名单里选：这个字符串来自面板，而由 UI 指定程序名去 `spawn`
等于命令注入。报「已安装」也以**能跑出版本号**为准，而不是文件存在就算：断掉的
符号链接或装一半的包否则会把失败推迟到「任务已经在等它」的那一刻。
...
可在应用内的「本机设置」窗口里改：设备名、默认工作目录（带系统目录选择器）、
任务可选的其他目录、审批方式、是否开机自动连接；站点地址折叠在
「改用其他站点（自托管）」里，
...
一台电脑上的任务不必共用一个目录，否则换个项目就得改本机设置再重连。建任务时
（云端任务没有这个选项，它跑在一次性沙箱的固定目录里）可以在执行位置旁边挑一个
目录，任务会记住它，之后每次启动运行时都用它当工作目录。

关键在于**选择权和授权权是分开的**：

- **客户端持有授权列表**——默认工作目录加上「本机设置」里添加的其他目录。服务器
  不能往这个列表里加东西，因为设备目标存在的意义就是让这条边界留在本机。
- **服务器只记录用户选了哪个目录**，启动时原样转发。客户端收到后重新检查它是否
  落在授权目录内（解析符号链接与 `..` 之后再比），不在就**拒绝启动**而不是退回
  默认目录——悄悄换个地方跑，会让任务一边报告成功一边改着没人让它碰的文件。
- **浏览目录也走同一条边界**。网页端的目录选择器不提供手输路径，它显示的每一层
  都由那台电脑通过自己的 socket 回答，且只回答授权范围内的路径；到了授权目录的
  根就不再往上走。所以这个接口既不能枚举别人的磁盘，也不会列出一个客户端随后
  必然拒绝的目录。

约束仍然由本机的审批门禁兜底：目录决定 Agent 能到哪里，审批决定它能不能动手。
...
# 要让任务可以挑多个项目，再加 YUNOVA_DEVICE_WORKSPACES=/srv/a:/srv/b
```
...
| `YUNOVA_DEVICE_WORKSPACES` | 无 | 任务还可以选的其他目录，冒号分隔（仅无界面模式；桌面模式在「本机设置」里维护） |
| `YUNOVA_DEVICE_NAME` | 主机名 | 列表里显示的名字 |
| `YUNOVA_DEVICE_AUTO_APPROVE` | `always` | 审批方式：`always` 逐条确认、`commands` 只确认命令、`never` 全部放行。仍兼容旧的 `1`/`0`（`1` 等于 `never`），谨慎使用 |
...
- **审批默认开启，且分三档**。客户端自己写入 `tool_call` 门禁 extension，
  指令可能来自手机，也可能受 Agent 读到的网页内容影响，所以在个人机器上
  「询问」才是安全默认值。三档而不是一个开关，是因为两端都不好用：一个改了
  二十个文件的任务会弹二十次框，用户随即学会闭眼点「允许」；而为了把活干完
  直接关掉门禁，等于连 shell 一起交出去。

  | 模式 | 拦住 | 适用 |
  | --- | --- | --- |
  | `always`（默认） | `bash`/`powershell`/`write`/`edit` | 不熟悉的任务、重要机器 |
  | `commands` | `bash`/`powershell` | 改动很多的任务：文件改动已被工作目录约束，命令不可撤销 |
  | `never` | 无 | 一次性、可丢弃的机器 |

  `read`/`grep`/`find` 任何一档都不拦：它们改不了机器，却会把真正该看的那一条
  淹没。门禁弹框会展示完整命令、写入内容或逐条改动，不展示内容的审批框只会
  训练出下意识的点击。模式存在本机，服务器只能读到客户端自报的值用于展示。

  单个任务可以在网页上另选一档，但**只能比这里更严**：客户端在启动运行时时
  取两者中更严的一个，所以一个要求「全部放行」的任务在这台机器上依旧会问。
  这条判断必须在客户端，不在服务器：否则能访问任务列表的人就能关掉你的确认。
- **工作目录限定范围**，默认当前目录而非 `$HOME`；不配置也不会默认暂开整个用户目录。
  任务可以在这些目录里挑一个具体项目，但**能挑哪些由客户端说了算**：服务器转发的
  目录要重新对授权列表校验（解析符号链接与 `..` 之后），不通过就拒绝启动。
...
常规用法是在建任务时直接选审批方式（或调 `POST /api/agent/sessions/{id}/approval`），
服务端会在启动容器前把对应的门禁写进这个目录。不选则云电脑全自动执行：
沙箱一次性、无外网、只能访问模型网关，这正是容器隔离存在的理由；
本地电脑则应始终保留审批。云电脑没有「自己的策略」可言，所以任务选什么就是什么；
本地电脑才有那个取更严的合成步骤。

手写一个更复杂的门禁也可以（需重启该会话的运行时生效）：
...
export default function (pi: ExtensionAPI) {
  pi.on("tool_call", async (event, ctx) => {
    ...
    // 注意两个容易写错的地方：参数在 `event.input`（不是 `args`），
    // `confirm` 收 (title, message) 两个位置参数（不是一个对象）。
    // 两者任一写错，弹框都会照常弹出、按钮照常能点，只是正文是空的——
    // 一个看不见命令的审批框比没有审批更危险。
    const ok = await ctx.ui.confirm("允许执行命令？", event.input.command)
...
手写的 extension 与任务选的审批方式会共存于同一目录，后者写入的文件名是
`yunova-approval.js`，别用这个名字以免被覆盖。
...
| `POST /api/agent/sessions/{id}/approval` | 设置该任务的审批方式（`always`/`commands`/`never`，`null` 表示跟随执行端）；**下次启动生效** |
...
- **审批方式也存在会话行上，但只能收紧，不能放松**。该不该逐条确认取决于任务
  而不是电脑：同一台笔记本既会跑批量改名，也会跑一段来路不明的脚本。所以每个
  任务可以自带一档审批，在合成时与执行端自己的策略取**更严**的一个
  （`ApprovalMode::strictest`）。方向是单向的：本机设成「逐条确认」时，任务要
  「全部放行」也仍然逐条确认；反过来，一台设成全放行的机器可以被单个任务要求
  确认。这条判断跑在客户端，因此即使服务器被攻破也改不动那台电脑的下限。
  网页上的下拉在被本机策略盖过时会直说是哪一档生效，而不是给一个点了不起作用的选项。
  合成后的策略在**运行时启动时**写成 extension，所以改动对已经跑起来的那一轮无效，
  控件也照此说明。
- **模型和级别改了立刻生效，审批方式不是**。前两者是 RPC 命令，可以对活着的
  运行时下发；审批门禁是运行时启动时加载的 extension，中途改只会让界面显示的
  策略和 Agent 实际执行的不一致——那比诚实地说「下次启动生效」更危险。
...
没有它就不能默认自动执行，而逐条手动确认工具调用等于没有 Agent。单个任务仍可以
主动选一档审批（见上），那是用户自己要求看一眼，而不是隔离不够的补偿。
